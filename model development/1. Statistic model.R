...
observed_PGT20 <- read_csv("df2.csv")

data_PGT20_observed <- data_fr  %>%
...
predictors <- data_PGT20_observed[,c(3:50)]
response <- data_PGT20_observed[,c(51)]
...
# Loading the model (set.seed(181))
models <- readRDS("Model_seed181.RData")
...
#11/811/181/118/1188/1/123//12//794/74/
...
rst1$category <- ifelse(rst1$Nt >= 2.2 & rst1$NSE >= 0.9, "Very Good",
                 ifelse(rst1$Nt >= 1.2 & rst1$Nt < 2.2 & 
                        rst1$NSE >= 0.8 & rst1$NSE < 0.9, "Good",
                 ifelse(rst1$Nt >= 0.7 & rst1$Nt < 1.2 & 
                        rst1$NSE >= 0.65 & rst1$NSE < 0.8, "Acceptable",
                 ifelse(rst1$Nt < 0.7 |rst1$NSE < 0.65, "Unsatisfactory", NA))))
...
top_10_percent <- allyears %>% 
  summarize(bot_10_MAE = quantile(MAE_mean, probs = .3,na.rm = TRUE),   # the value of the 30th percentile of the MAE
            bot_10_RMSE = quantile(RMSE_mean, probs = .3,na.rm = TRUE), # the value of the 30th percentile of the RMSE
            top_10_r2 = quantile(Rsqared_mean, probs = .7,na.rm = TRUE))     # the value of the 70th percentile of the R2

# Selected model based on RMSE, MAE, and R-squared 
Selected_model <- rst1 %>%  
  filter(category %in% c("Good", "Very Good")) %>%
  filter(MAE <= top_10_percent$bot_10_MAE,
         RMSE <= top_10_percent$bot_10_RMSE,
         r2 >= top_10_percent$top_10_r2) 
...
write_csv(Selected_model, "Selected_model.csv")
mdls2[[794]]
...
write_csv(aydata, file = "your_data2_ne.csv")
...
# save the model
aydata_acceptable$model2 <- sapply(aydata_acceptable$model, function(x) paste(x, collapse = ","))
variable_sel_step[Acceptable_Model$Model_Number]
write_csv(aydata_acceptable, file = "acceptable181.csv")
...
r_181 <- data.frame(a1 = aydata_acceptable$RMSE_mean,
                    a2 = aydata_acceptable$Rsqared_mean,
                    a3 = aydata_acceptable$MAE_mean,
                    b1 = hidata_acceptable$RMSE_mean,
                    b2 = hidata_acceptable$Rsqared_mean,
                    b3 = hidata_acceptable$MAE_mean, 
                    g1 = lidata_acceptable$RMSE_mean,
                    g2 = lidata_acceptable$Rsqared_mean,
                    g3 = lidata_acceptable$MAE_mean)

write_csv(r_181, "R2_acceptable181.csv")
...
f2_1 <- ggplot(fig2data, mapping=aes(x=factor(year),
  ...
  labs(title='Average daily maximum temperatures \n(°C)')+
  theme_classic() +
...
f2_2 <- ggplot(fig2data, mapping=aes(x=factor(year),
  ...
  geom_boxplot() +
  scale_fill_manual(values = c("#ffffff","#999999")) +
  labs(title='Average daily minimum temperatures \n(°C)')+
  theme_classic() +
  theme(legend.position = 'none',
        axis.text.y = element_text(family = "Arial", size = 12),
        axis.text.x =element_blank(),
...
f2_3 <- ggplot(fig2data, mapping=aes(x=factor(year),
  ...
  theme_classic() +
  scale_fill_manual(values = c("#ffffff","#999999")) +
  labs(title='Average of average daily temperatures \n(°C)')+
...
f2_4 <- ggplot(fig2data, mapping=aes(x=factor(year),
  ...
  theme_classic() +
  scale_fill_manual(values = c("#ffffff","#999999")) +
  labs(title='Accumulated rainfall \n(mm)')+
  theme(axis.title = element_blank(),
        axis.text.y = element_text(family = "Arial", size = 12),
        axis.text.x =element_blank(),
        legend.position = 'none',
...
f2_5 <- ggplot(fig2data, mapping=aes(x=factor(year),
  ...
  theme_classic() +
  scale_fill_manual(values = c("#ffffff","#999999")) +
  labs(title='Aflatoxin Concentration \n(μg/kg, ppb)')+
  theme(axis.title = element_blank(),
        axis.text.y = element_text(family = "Arial", size = 12),
        axis.text.x =element_text(family = "Arial", size = 12),
        legend.position = 'none',
...
f2_6 <- ggplot(fig2data, mapping=aes(x=factor(year),
  ...
  theme_classic() +
  scale_fill_manual(values = c("#ffffff","#999999")) +
  labs(title='Percentage of samples over 20 ppb \naflatoxin (PGT20,%)')+
  theme(axis.title = element_blank(),
        axis.text.y = element_text(family = "Arial", size = 12),
        axis.text.x = element_text(family = "Arial", size = 12),
        legend.position = 'none',
...
ggsave(plot = f2, filename = "figure2_181.jpg",
...
f3 <- ggplot(allyears, aes(x = Nt_mean,
  ...
  scale_y_continuous(limits = c(min(allyears$Nt_mean), 
                     ...
                     breaks=c(0,0.65)) +
  scale_x_continuous(breaks=c(0,0.7))+
  scale_shape_manual(values = c(1, 4)) +
  scale_color_manual(values = c("blue","grey"))+
  labs(y ='Nash-Sutcliffe coefficient Efficiency (NSE)',
...
f3
ggplot(rst1 , aes(x = Nt,
                     y = NSE,
                     color = category,
                     shape = category)) +
  geom_point(show.legend = FALSE) +
  scale_y_continuous(limits = c(min(rst1$Nt), 
                                max(rst1$Nt)),
                     breaks=c(0,0.65,0.8, 0.9)) +
  scale_x_continuous(breaks=c(0,0.7,1.2,2.2))+
  scale_shape_manual(values = c(1, 4,2,5)) +
  scale_color_manual(values = c("blue","grey","black", "green"))+
  labs(title='Nash-Sutcliffe coefficient Efficiency (NSE)',
       x = "Number of times that observed data dispersion is larger than prediciton error (Nt)")+
  theme_graph +
  geom_hline(yintercept= 0.65, linetype='dashed', color = 'red') +
  geom_vline(xintercept= 0.7, linetype='dashed', color = 'red') +
  annotate("text", x=0.7, y=0, color ="black", label="Unsatisfactory",family = "Arial")+
...
ggsave(plot = f4, filename = "figure4_.jpg",
...
## graph pgt 20 model4
FinalModel <- mdls2[[794]]

fig5_1 <- data.frame(x = data_fr$PGT20, 
...
f5_1 <- ggplot(fig5_data,
  ...
  theme(text = element_text(family = "Arial", size = 12),
        ...
        axis.text.x = element_text(family = "Arial", size = 12),
        plot.title = element_text(family = "Arial", size = 12)) +
  annotate("text", x=25, y=27, color ="#003f5c", label="All 5 years",family = "Arial")+
  annotate("text", x=30, y=15, color ="#bc5090", label="High Incidence \n years",family = "Arial")+
  annotate("text", x=15, y=3, color ="#ffa600", label="Low Incidence years",family = "Arial")
...
fig5 <- plot_grid(f5_1, f5_2,
                  labels = c('(a)','(b)'), 
                  nrow = 1, label_x = -0.03)

fig5

ggsave(plot = fig5, filename = "figure5_181.png",
...
fig6_1 <- data.frame(x = data_fr$PGT20, 
...
f6_1 <- ggplot(fig6_data,
  ...
  theme(text = element_text(family = "Arial", size = 12),
        ...
        axis.text.x = element_text(family = "Arial", size = 12),
        plot.title = element_text(family = "Arial", size = 12)) +
  annotate("text", x=20, y=45, color ="#003f5c", label="All 5 years",family = "Arial")+
  annotate("text", x=25, y=20, color ="#bc5090", label="High Incidence \n years",family = "Arial")+
  annotate("text", x=15, y=3, color ="#ffa600", label="Low Incidence years",family = "Arial")
...
ggsave("figure6_181.png", plot = fig6,
        width = 174, height = 80, units = 'mm')
...
# save the model
aydata_acceptable_4$model2 <- sapply(aydata_acceptable_4$model, function(x) paste(x, collapse = ","))
variable_sel_step[Acceptable_Model$Model_Number]
write_csv(aydata_acceptable_4, file = "acceptable181_pgt4.csv")
...
#model fit 
mdls2[[794]]
model <- lm(PGT20 ~ MinT4wk + MinT2wk + MaxT5wk + Rain4wk + MaxT4wk, data = data_al_p)
summary(model)

install.packages('relaimpo')
library(relaimpo)
partial_plot <- partial(model, pred.var =  "MaxT4wk", grid.resolution = 10)
plot(partial_plot)


relimp<- calc.relimp(model, type = c("lmg","last","first","pratt"),
                     importance = T)

relimp<- calc.relimp(model, type = c("lmg"),
                     importance = T)

plot(relimp)


boot <- boot.relimp(model, b = 1000, type = c("lmg"), rank = TRUE,
                    diff = TRUE, rela = TRUE)

ci<- booteval.relimp(boot, norank = T) # print result

plot(booteval.relimp(boot,sort=TRUE)) # plot result


plot <- data.frame(
  Variable =c("MinT4wk", "MinT2wk","MaxT5wk", "Rain4wk", "MaxT4wk"),
  mean = c(0.1708146, 0.1763479, 0.2879872, 0.09756275, 0.2672876) *100,
  lower = c(0.1215125, 0.1143205, 0.1918437, 0.04462697, 0.1727218)*100,
  upper = c(0.2342184, 0.2670899, 0.3467903, 0.19446636, 0.3281858)*100
)
plot$mean *100

fonts()
# Create the plot
figure7 <- ggplot(plot, aes(x = Variable, y = mean,alpha=0.7)) +
  geom_col() +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.1) +
  labs( x = "Variable", y = "% of R-squared") +
  theme_classic()+
  theme(text = element_text(family = "Arial", size = 12),
        axis.title.y = element_text(family = "Arial", size = 12),
        legend.position ='none',
        ...
        axis.text.x = element_text(family = "Arial", size = 12),
        plot.title = element_text(family = "Arial", size = 12))
ggsave("figure7.png", plot = figure7, 
